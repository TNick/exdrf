"""GUI helpers for planning PDF split operations."""
...
if TYPE_CHECKING:  # pragma: no cover - type checking only
    from exdrf_qt.controls.pdf_viewer.pdf_image_viewer import (
        PdfImageViewer as PdfImageViewerType,
    )

logger = logging.getLogger(__name__)


__all__ = ["SplitEntry", "SplitPlanPanel"]
...
class SplitPlanPanel(QWidget):
    ...
    def __init__(self, viewer: "PdfImageViewerType"):
...
if __name__ == "__main__":
    ...
    if os.environ.get("EXDRF_TESSERACT_PATH"):
        import pytesseract  # type: ignore[import-untyped]

        pytesseract.pytesseract.tesseract_cmd = os.environ.get(
            "EXDRF_TESSERACT_PATH"
        )
    ...
    top_widget = QWidget()
    top_widget.mdi_area = None  # type: ignore[attr-defined]
    # Note: c_string and schema are DbConn parameters (parent class)
    ctx = QtContext(  # type: ignore[call-arg]
        c_string="",
        stg=LocalSettings(),  # type: ignore[arg-type]
        top_widget=top_widget,  # type: ignore[arg-type]
    ...
    try:
        ...
        QMessageBox.critical(viewer, "Error", str(exc))
        sys.exit(1)
