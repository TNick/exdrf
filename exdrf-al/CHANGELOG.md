# Changelog





## [Unreleased]

## [0.1.16] - 2026-05-14

## [0.1.15] - 2026-05-13

### Changed

- Update description

## [0.1.14] - 2026-05-13

### Changed

- Add initial libraries
- Load from sqlalchemy
- Fix formatting
- import using API. Start the work on pydantic loader
- Context and worker in qt
- Created dev
- app in qt
- Tests for al base
- editors
- Package for generating content and two stub plugins
- Compute the optimized query for models
- can change some in generated code. Single and multi editors not saving
- Fixing enums
- Show enums in list
- Prepare release 0.0.1
- tag_format is unknown
- Fix stringify for nested queries
- Bootstrap button in configuration
- Remove relations from load_only
- Preserve portions of the generated template
- db toold, read-only and buddy
- fix sql statement geneerators
- Better sorting for fields
- Some more work on filters
- Utilities for database migrations
- Adjust alembic impl
- improve qt actions
- deletion in router
- Editor in selector
- Added top-cache
- count_relationship and other fixes
- Allow choices in integer, float and string fields
- added filter header
- Toast and the proxy model
- exdrf-qt: enhance model system with improved field handling and comprehensive tests
- Improve template editor
- Allow for non-column fields like date.year
- trap missing secondary in many to many
- Added excel i/o module
- Check support
- Better query constructor
- Showing some love to the qt model
- Delete-aware model
- Added some gui elements, save model settings
- Update the connection settings dialog
- update the handling of read-only fields
- Added concepts
- Introducing new search lines
- Fix search lines
- Some rogress for the ref editor
- Working relational editor
- Load precision in ui for real/float editor. Visit hybrid properties.
- added docstrings
- addedd connection parameters
- Ignore missing version table
- Use verbose
- Multiple improvements while working on the web app
- All tests are now green
- Remove deprecated version_path_separator. Use default_factory for pydantic
- Create and update in fapi routes
- First implementation of lists in fapi
- added more temates/plugins for generator
- Sync local sttings
- Add ability to adjust o2m output
- Changes after running tests in github
- actions trouble
- Check uniformization across projects
- Fix mypy errors
- Fix a bunch of mypy errors. Change to unique pypi token. Fix license metadata.
- Fix license warning. More pypy fixes
- fix scm version
- Break the dependency on qt
- remove dynamic versioning

[0.1.14]: https://github.com/TNick/exdrf/compare/5c63497ca9407e723f6504245ac324094d46b6a6...v0.1.14-exdrf-al
[0.1.15]: https://github.com/TNick/exdrf/compare/v0.1.14-exdrf-al...v0.1.15-exdrf-al
[0.1.16]: https://github.com/TNick/exdrf/compare/v0.1.15-exdrf-al...v0.1.16-exdrf-al
[unreleased]: https://github.com/TNick/exdrf/compare/v0.1.16-exdrf-al...HEAD
