...
class QtChildCmp(RecordComparatorBase):
    """Compare/merge widget for Child records.
    ...
    def __init__(
        ...
        if not self.windowTitle():
            self.setWindowTitle(
                self.t("child.cmp.title", "Child compare"),
            )

        adapters: List[RecordToNodeAdapter] = []
        if record_ids:
            from exdrf_dev.qt_gen.db.children.models.child_ful import (
                QtChildFuMo,
            )

            ful = QtChildFuMo(ctx=ctx, prevent_total_count=True)
            field_map = {f.name: f for f in ful.fields}

            with self.ctx.same_session() as session:
                for i, rec_id in enumerate(record_ids):
                    rec = self._load_record(session, DbChild, rec_id)
                    if rec is not None:
                        name = self.t(
                            "child.cmp.record_label",
                            "Record {n}",
                            n=i + 1,
                        )
                        adapters.append(
        ...
        safe_hook_call(
            exdrf_qt_pm.hook.child_cmp_created,
            widget=self,
        )
    ...
    def get_viewer_class(self) -> Type["RecordTemplViewer"]:
        from .child_tv import QtChildTv
