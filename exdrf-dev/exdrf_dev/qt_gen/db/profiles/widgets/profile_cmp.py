...
class QtProfileCmp(RecordComparatorBase):
    """Compare/merge widget for Profile records.
    ...
    def __init__(
        ...
        if not self.windowTitle():
            self.setWindowTitle(
                self.t("profile.cmp.title", "Profile compare"),
            )

        adapters: List[RecordToNodeAdapter] = []
        if record_ids:
            from exdrf_dev.qt_gen.db.profiles.models.profile_ful import (
                QtProfileFuMo,
            )

            ful = QtProfileFuMo(ctx=ctx, prevent_total_count=True)
            field_map = {f.name: f for f in ful.fields}

            with self.ctx.same_session() as session:
                for i, rec_id in enumerate(record_ids):
                    rec = self._load_record(session, DbProfile, rec_id)
                    if rec is not None:
                        name = self.t(
                            "profile.cmp.record_label",
                            "Record {n}",
                            n=i + 1,
                        )
                        adapters.append(
        ...
        safe_hook_call(
            exdrf_qt_pm.hook.profile_cmp_created,
            widget=self,
        )
    ...
    def get_viewer_class(self) -> Type["RecordTemplViewer"]:
        from .profile_tv import QtProfileTv
