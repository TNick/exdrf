...
class QtTagCmp(RecordComparatorBase):
    """Compare/merge widget for Tag records.
    ...
    def __init__(
        ...
        if not self.windowTitle():
            self.setWindowTitle(
                self.t("tag.cmp.title", "Tag compare"),
            )

        adapters: List[RecordToNodeAdapter] = []
        if record_ids:
            from exdrf_dev.qt_gen.db.tags.models.tag_ful import (
                QtTagFuMo,
            )

            ful = QtTagFuMo(ctx=ctx, prevent_total_count=True)
            field_map = {f.name: f for f in ful.fields}

            with self.ctx.same_session() as session:
                for i, rec_id in enumerate(record_ids):
                    rec = self._load_record(session, DbTag, rec_id)
                    if rec is not None:
                        name = self.t(
                            "tag.cmp.record_label",
                            "Record {n}",
                            n=i + 1,
                        )
                        adapters.append(
        ...
        safe_hook_call(
            exdrf_qt_pm.hook.tag_cmp_created,
            widget=self,
        )
