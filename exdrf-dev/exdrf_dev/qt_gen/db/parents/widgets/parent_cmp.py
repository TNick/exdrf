...
class QtParentCmp(RecordComparatorBase):
    """Compare/merge widget for Parent records.
    ...
    def __init__(
        ...
        if not self.windowTitle():
            self.setWindowTitle(
                self.t("parent.cmp.title", "Parent compare"),
            )
        ...
        if record_ids:
            ...
            ful = QtParentFuMo(ctx=ctx, prevent_total_count=True)
            field_map = {f.name: f for f in ful.fields}

            with self.ctx.same_session() as session:
                for i, rec_id in enumerate(record_ids):
                    rec = self._load_record(session, DbParent, rec_id)
                    if rec is not None:
                        name = self.t(
                            "parent.cmp.record_label",
                            "Record {n}",
                            n=i + 1,
                        )
                        adapters.append(
        ...
        safe_hook_call(
            exdrf_qt_pm.hook.parent_cmp_created,
            widget=self,
        )
    ...
    def get_viewer_class(self) -> Type["RecordTemplViewer"]:
        from .parent_tv import QtParentTv
